...
class DeepwalkWithRegularisation(object):


    def __init__(self, walk_number=5, walk_length=80, dimensions=16, negative_samples=10,
                 window_size=5 , learning_rate_initial = 0.01, learning_rate_final = 0.001, Lambda = 2.0**-4):
    ...
    def _do_descent_for_pair(self, graph, negative_samples, source_node, target_node):
        ...
        node_gradient = noise_vector - target_vector + self.Lambda*weight*regularisation_vector
    ...
    def save_results(self, path):
        ...
        final_embeddings.to_csv(path + '\DWR_embeddings.csv', index=None)
        c_means = pd.DataFrame(self._cluster_centers)
        c_means.to_csv(path + '\DWR_cluster_means.csv', index=None)
        with open(path+'\DWR_memberships.txt', "w") as outfile:
