...
class GEMSEC(object):
    ...
    def __init__(self, walk_number=5, walk_length=80, dimensions=16, negative_samples=10,
                 ...
                 learning_rate_initial = 0.1, learning_rate_final = 0.001):
    ...
    def _do_descent_for_pair(self, graph, negative_samples, source_node, target_node):
        ...
        node_gradient = noise_vector - target_vector + self.current_gamma*cluster_vector 
    ...
    def save_results(self, path):
        ...
        final_embeddings.to_csv(path + '\GM_embeddings.csv', index=None)
        c_means = pd.DataFrame(self._cluster_centers)
        c_means.to_csv(path + '\GM_cluster_means.csv', index=None)
        with open(path+'\GM_memberships.txt', "w") as outfile:
